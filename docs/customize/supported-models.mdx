...
### OpenRouter
[OpenRouter](https://openrouter.ai) is a unified API platform that provides access to hundreds of AI models from various providers through a single API. This allows you to easily switch between models from OpenAI, Anthropic, and many others.
...
llm = ChatOpenAI(
    ...
    model="openai/gpt-4o",  # Use any supported model ID (e.g., "anthropic/claude-3-5-sonnet")
    api_key=SecretStr(api_key),
    model_kwargs={
        "headers": {
            "HTTP-Referer": os.getenv("SITE_URL", ""),  # Optional: for analytics
            "X-Title": os.getenv("SITE_NAME", "Browser Use"),  # Optional: for analytics
        }
    },
)
